...
# 生成噪声图像
def generate_noise_image(noise_level, noise_type, shape=(28, 28)):
    ...
    if noise_type == '高斯噪声':
        ...
        stripe_value = np.random.normal(0, noise_level)
        for i in range(shape[0]):
            if i % 2 == 0:
                noise_image[i, :] = stripe_value
    elif noise_type == '椒盐噪声':
        noise_image = np.random.choice([0, 1], size=shape, p=[1 - noise_level, noise_level])
    return noise_image
# 生成28x28的像素矩阵
...
if digit_image is not None:
    ...
    noise_type = col2.selectbox('选择噪声类型', ['高斯噪声', '网格噪声', '条纹噪声', '椒盐噪声'])
    noise_image = generate_noise_image(noise_level, noise_type)
