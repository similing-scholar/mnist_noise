...
# 生成噪声图像
def generate_noise_image(noise_level, noise_type, shape=(28, 28), logo_path=None, logo_alpha=0.5, background_path=None):
    ...
    if noise_type == 'percent高斯噪声':
        ...
        noise_image = _generate_logo_noise(shape, logo_path, logo_alpha, 1)
    elif noise_type == '背景噪声' and background_path:
        noise_image = _generate_background_noise(shape, background_path, 1)
    return noise_image
...
# 生成logo噪声
def _generate_logo_noise(size, logo_path, logo_alpha, luminance_max):
    logo = Image.open(logo_path).convert("L")
    logo = logo.resize((size[1], size[0]))
    logo_array = np.array(logo) / 255.0  # 将logo灰度值归一化到[0, 1]
    return logo_array * logo_alpha * luminance_max  # 将logo灰度值乘以透明度因子并匹配亮度最大值

# 生成背景噪声
def _generate_background_noise(size, background_path, luminance_max):
    background = Image.open(background_path).convert("L")
    background = background.resize((size[1], size[0]))
    background_array = np.array(background) / 255.0  # 将背景图片灰度值归一化到[0, 1]
    return background_array * luminance_max  # 返回匹配亮度最大值的背景噪声

# 生成28x28的像素矩阵
def generate_pixel_matrix(digit_image, luminance, chosen_time_index, noise_image, apply_clipping=True):
...
if digit_image is not None:
    ...
    chosen_luminance = luminance[chosen_time_index]  # 获取当前所选时间点的亮度值
    col2.write(f"当前所选时间点的亮度值: {chosen_luminance}")
    noise_level = col2.number_input('噪声强度', min_value=0, value=2)
    noise_type = col2.selectbox('选择噪声类型', ['percent高斯噪声', '网格噪声', '条纹噪声', '马赛克噪声', 'logo噪声', '背景噪声'])
    logo_path = 'D:/BITcode/__code__/PJ_LML/dataset/BIT.jpg' if noise_type == 'logo噪声' else None
    background_path = 'D:/BITcode/__code__/PJ_LML/dataset/BIT.jpg' if noise_type == '背景噪声' else None
    logo_alpha = col2.slider('Logo透明度', min_value=0.0, max_value=1.0, value=0.5) if noise_type == 'logo噪声' else 0.5

    # 如果选择了背景噪声或logo噪声，将亮度最大值传递给生成函数
    if noise_type == 'logo噪声':
        noise_image = _generate_logo_noise((28, 28), logo_path, logo_alpha, max(luminance))
    elif noise_type == '背景噪声':
        noise_image = _generate_background_noise((28, 28), background_path, max(luminance))
    else:
        noise_image = generate_noise_image(noise_level, noise_type, logo_path=logo_path, logo_alpha=logo_alpha, background_path=background_path)

    # 添加亮度值限制选项
    apply_clipping = col2.checkbox('将亮度值限制在0-1之间', value=False)

    # 生成像素矩阵
    pixel_matrix = generate_pixel_matrix(digit_image, luminance, chosen_time_index, noise_image, apply_clipping)
