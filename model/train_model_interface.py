...
class StepRunner:
    ...
    def __call__(self, features, labels):  # __call__使实例能够像函数一样被调用
        """用于执行一步训练或验证
        包括计算损失、反向传播、参数更新（仅在训练阶段）、计算指标，并返回损失和指标的字典。
        """
        # 移动数据到cuda
        features = features.to(self.device, self.dtype)
        labels = labels.to(self.device, torch.long)  # 分类任务，确保标签是long类型！！！
...
class KerasModel(torch.nn.Module):
    ...
    def __init__(self, device, dtype, net, loss_fn, metrics_dict=None, optimizer=None, lr_scheduler=None, weight_decay=0.0):
        super().__init__()
        ...
        self.optimizer = optimizer if optimizer is not None else torch.optim.Adam(
            self.parameters(), lr=1e-2, weight_decay=weight_decay)
